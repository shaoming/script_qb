...
class GolbalSetting:
	def __init__(self,walltime,work):
		self.PBS_PARAM='#!/bin/bash'+"\n"
                self.PBS_PARAM+='#PBS -A loni_metrics_16 '+"\n"
                self.PBS_PARAM+='#PBS -q checkpt'+"\n"
                self.PBS_PARAM+='#PBS -l nodes=1:ppn=20'+"\n"
                self.PBS_PARAM+='#PBS -l walltime=48:00:00'+"\n"
		self.line ='-A loni_metrics_16 -q checkpt -l nodes=1:ppn=20 -l walltime='+str(walltime)+':00:00  '
		if not os.path.exists(work):
			print "the path do not exists : ",work
			sys.exit(1)
		self.work= work
	def getPBS_para(self,log,sname):
		script = self.PBS_PARAM + '#PBS -o '+sname+'.output'+" \n" 
		script += log
		#f = open(sname,'w')
		#f.write(script)
		#f.close()
...
for bench in benches:
	#for j in range(len(benchs)):
		...
		#bench_script = SPECexcuteScript(path,"rc","",bench,"--L2fetch --Degreefetch 2 --sub_channel 3  --core_freq 4GHz")
		#bench_script = SPECexcuteScript(path,"rc","",bench,"--memfetch --L2fetch --Degreefetch 4 --sub_channel 3  --core_freq 2.4GHz")
		...
		#script = bench_script.getScript("./script/",500,2*(10**8),"MSM_PCM_P2_C3_4G")
		#script = bench_script.getScript("./script/",500,2*(10**8),"MSM_PCM_C3_4G")
		#script = bench_script.getScript("./script/",500,2*(10**8),"MSM_PCM_C3_24G")
		#script = bench_script.getScript("./script/",500,2*(10**8),"MSM_PCM_P4_C3_24G")
		#script = bench_script.getScript("./script/",500,2*(10**8),"MSMC3_24GRED")
		...
		pbs = PBS.getPBS_para(script," ")
       		print "command line ",pbs
		#subprocess.call("echo $PBS_PARAM", shell=True)
		cmd = 'qsub '+pbs+"\n"
		cmd = 'qsub '+script+"\n"
		#os.chmod(script,stat.S_IEXEC | os.stat(script).st_mode)
