...
class GolbalSetting:
	def __init__(self,walltime,work):
		self.PBS_PARAM='-A loni_metrics_15 -q workq -l nodes=1:ppn=10 -l walltime='+str(walltime)+':00:00  '
	...
	def getPBS_para(self,log):
		...
		return var
class ParsecScript:
	def __init__(self,path,bench,num_core):
		self.path = path
		self.bench = bench
		#self.size = size
		self.num_core = num_core
	def getScript(self,path,fastforward, inst,script_path,suffix,input):
		filename = path+self.bench+"_"+str(self.num_core)+suffix+".sh"
		if not os.path.exists(path):
			os.makedirs(path)
		try:
			f = open(filename,"w")
		...
		f.write("source rc "+"\n")
		reportfile = self.bench+"_"+str(self.num_core)+suffix+".txt"
		...
		script_name = self.bench+"_"+str(self.num_core)+".rcS" 
...
if True:
	for bench in benchs:
		...
		bench_script = ParsecScript(path,bench,num_core)
		...
		script = bench_script.getScript("./script/",2000,2*(10**8),script_path,"SPLASH2_MC3G24","--memfetch  --conf 2 --sub_channel 3 --core_freq 2.4GHz")
		#script = bench_script.getScript("./script/",2000,2*(10**8),script_path,"SPLASH2_C3G4"," --conf 2 --sub_channel 3 --core_freq 4GHz")
