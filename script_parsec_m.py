...
class GolbalSetting:
	def __init__(self,walltime,work):
		self.PBS_PARAM='-A loni_metrics_15 -q workq -l nodes=2:ppn=20 -l walltime='+str(walltime)+':00:00  '
...
class ParsecScript:
	...
	def getScript(self,benchs,path,fastforward, inst,script_path,suffix,input):
		...
		cmd =""
		for bench in benchs:
			reportfile = bench+"_"+str(self.num_core)+"c_"+self.size+suffix+".txt"
			...
			script_name = bench+"_"+str(self.num_core)+"c_"+self.size+".rcS" 
...
if True:
	if True:
		...
		script = bench_script.getScript(benchs,"./script/",2000,2*(10**8),script_path,"MSTATC3G24","--memfetch  --conf 2 --sub_channel 3 --core_freq 2.4GHz &")
